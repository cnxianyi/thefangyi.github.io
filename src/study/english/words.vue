<template>
  <div>
    <nav-bar>
      <template v-slot:default> English Words </template>
      <template v-slot:time> 2021年8月15日20:12:10 </template>
    </nav-bar>
    <div class="xy">
      <div class="content">
        <div class="button">
...
export default {
  ...
  methods: {
    ...
    fun() {
      if (
        (this.addWords.length >= 1) &
        (this.addPos.length >= 1) &
        (this.addMeaning.length >= 1)
      ) {
...
.button {
  ...
  .btn {
    ...
    box-shadow: rgba(151, 57, 57, 0.363) 0px 15px 25px,
      rgba(156, 57, 57, 0.425) 0px 5px 10px;
    min-width: 40px;
    max-width: 120px;
    min-height: 35px;
